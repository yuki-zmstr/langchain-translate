...
with row1_left:
    # print("source" in st.session_state)  # => False
    ...
    # print("source" in st.session_state)  # => True
...
def translate(source: str, target: str, input: str) -> str:
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", "次の{source_language}を{target_language}に翻訳して下さい"),
        ("user", "{user_input}")
    ])
...
with row2_right:
    if input != "":
        with st.spinner("翻訳中。。。"):
            st.write(translate(source, target, input))
